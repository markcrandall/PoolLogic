...
// load transition, same rule the store follows for the connection FSM. Each
// carries its own in-flight latch too — see resource.js, which the panel view
// now shares.
//
// Re-render explicitly on arrival: a fetch is not a state transition and will
// not notify subscribers on its own.
const rerender = () => render(store.getState());
const configRes = createResource(fetchConfig, rerender);
const panelRes = createResource(fetchPanelInfo, rerender);
...
export function afterPoll() {
  ...
  // not be final, which is what it used to be. A dropped fetch recovers on the
  // next poll and a map already in hand is never re-requested. (Overlap is the
  // resource's latch, not this call's; see resource.js.)
